...
class TreeNode:
    ...
    def __init__(self, data: any, previous=None):
        """
		Creates a node with the values data, and children

		Args:
			data(any): data that will be stored in the tree node.
            previous(TreeNode): A node that will be set as the previous node
                is optional.
		"""
        self.data = data
    ...
    while total_nodes < len(data) or not nodes_to_be_filled.is_empty():
        ...
        if current_node.left is None and current_node.right is None:
            if total_nodes == len(data):
                ...
                current_node.left = TreeNode(None, previous=current_node)
                current_node.right = TreeNode(None, previous=current_node)
                total_nodes += 2
